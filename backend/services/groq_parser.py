...
def _call_grow(client:Groq, system_prompt:str, user_prompt:str) -> str:
    
    response = client.chat.completition.create(
...
def parse_resume(raw_text: str) -> Dict:
    client = get_client()
    prompt = RESUME_USER_PROMPT.fromat(raw_text = raw_text)
    ...
    raw_response = call_groq(client, RESUME_SYSTEM_PROMPT, strict_prompt)
    result = _try_parse_json(raw_response)
    if result is not None:
        return _validate_resume_result(result)

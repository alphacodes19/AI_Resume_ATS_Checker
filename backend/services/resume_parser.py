...
def extract_text_from_pdf(file_data: bytes) -> str:
    try: result, used_fallback = with_fallback(
    ...
    if used_fallback:
            log_info('PDF EXTRACTION succeed using the PYPDF2 fallback', context = 'resume_parser')
            return result
...
def extract_text_from_docx(file_data: bytes) -> str:
    try:
        ...
        raise FileParsingError(
            'Failed to extract text from DOCS'
            'The document may be corrupted or in as unsupported format'
...
def extrafct_text(file_data: bytes, file_type: str) -> str:
    if file_type == 'pdf':
